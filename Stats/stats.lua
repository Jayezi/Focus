...
local data_parent = FocusDataPanel
...
local mem_text = core.util.gen_string(data_parent, core.config.font_size_med)
mem_text:SetPoint("BOTTOMLEFT", data_parent, "BOTTOMLEFT", 7, 5)

local mem_frame = CreateFrame("Button", "FocusStatsMem", UIParent)
...
local friend_text = core.util.gen_string(data_parent, core.config.font_size_med)
friend_text:SetPoint("BOTTOMLEFT", mem_text, "BOTTOMRIGHT", 5, 0)

local friend_frame = CreateFrame("Button", "FocusStatsFriends", UIParent)
...
friend_frame:SetScript("OnEvent", function(self, event)
	...
	friend_text:SetText(string.format("%d|c%sf|r", online_friends + online_bn_friends, core.player.color_str))
...
local net_text = core.util.gen_string(data_parent, core.config.font_size_med)
net_text:SetPoint("BOTTOMLEFT", friend_text, "BOTTOMRIGHT", 5, 0)

local net_frame = CreateFrame("Frame", "FocusStatsNet", UIParent)
...
net_frame:SetScript("OnEvent", function(self, event)
	...
	net_text:SetText(string.format("%s|c%sh|r %s|c%sw|r", home, core.player.color_str, world, core.player.color_str))
end)
...
local fps_text = core.util.gen_string(data_parent, core.config.font_size_med)
...
fps_frame:SetScript("OnEvent", function(self, event)
	...
	fps_text:SetText(string.format("%d|c%sfps|r", floor(fps), core.player.color_str, "fps"))
end)
...
local format_gold = function(rawgold)
	return string.format("%d|c%sg|r", floor(rawgold * 0.0001), core.player.color_str)
end

local gold_text = core.util.gen_string(data_parent, core.config.font_size_med)
...
gold_frame:SetScript("OnEnter", function(self)
	GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
	GameTooltip:ClearLines()
	GameTooltip:AddLine(format("Gold on |c%s%s|r", core.player.color_str, core.player.realm), 1, 1, 1)
	GameTooltip:AddLine" "
	local sum = 0
...
local azerite_text = core.util.gen_string(data_parent, core.config.font_size_med)
azerite_text:SetPoint("BOTTOMRIGHT", gold_text, "BOTTOMLEFT", -5, 0)

local azerite_frame = CreateFrame("Frame", "FocusStatsAzerite", UIParent)
...
azerite_frame:SetScript("OnEvent", function(self, event)
	...
	azerite_text:SetText(format("%s - %d|c%s%s|r", self.currentLevel, floor(100 * self.xp / self.totalLevelXP), core.player.color_str, "%"))
end)

azerite_frame:SetScript("OnEnter", function(self)
	GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
	GameTooltip:ClearLines()
	GameTooltip:AddLine(format("Level |c%s%d|r", core.player.color_str, self.currentLevel), 1, 1, 1)
	GameTooltip:AddLine(format("%d / %d", self.xp, self.totalLevelXP), 1, 1, 1)
	GameTooltip:AddLine(format("|c%s%d|r to next level", core.player.color_str, self.xpToNextLevel), 1, 1, 1)
	GameTooltip:AddLine" "
	...
	for id, name in pairs(reps) do
		if (C_Reputation.IsFactionParagon(id)) then
			...
			GameTooltip:AddDoubleLine(name, format("|c%s%d / %d|r", core.player.color_str, value, threshold), 1, 1, 1, 1, 1, 1)
		end
	end
