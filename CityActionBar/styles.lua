...
styles.actionbutton = function(button, bar_cfg)
	...
	cui.util.gen_backdrop(button, unpack(cui.config.frame_background_transparent))
	...
	cui.util.fix_string(cooldown:GetRegions(), bar_cfg and bar_cfg.cooldown_size or cui.config.font_size_lrg)
	...
	cui.util.fix_string(hotkey, cui.config.font_size_med)
	...
	cui.util.fix_string(count, cui.config.font_size_lrg)
...
styles.microbutton = function(button)
	...
	flash:SetTexture(cui.media.textures.blank)
	flash:SetVertexColor(unpack(cfg.color.flash))
	cui.util.set_inside(flash, button, inset)
	...
	button:SetBackdropColor(unpack(cui.config.frame_border))
end
