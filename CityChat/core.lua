...
local skin_chat_frame = function(frame)
	...
	frame_bg:SetVertexColor(unpack(cui.config.frame_background_transparent))
	...
	for _, tex_name in pairs(frame_border_textures) do
		...
		texture:SetSize(1, 1)
		texture:SetVertexColor(unpack(cui.config.frame_border))
	...
	cui.util.fix_string(tab_fs, cui.config.font_size_med)
	...
	hooksecurefunc(frame, "Show", function(self)
		tab_fs:oldSetTextColor(unpack(cui.player.color))
	end)
...
cui.util.set_inside(normal, ChatFrameMenuButton)
...
cui.util.set_inside(highlight, ChatFrameMenuButton)
...
cui.util.fix_string(ChatFontNormal, cui.config.font_size_med)
